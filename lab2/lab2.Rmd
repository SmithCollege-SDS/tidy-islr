...
Now let's fit a simple linear model with `medv` as the response and `lstat` as the predictor
...
```{r}
predict(lm.fit,new_values, interval="confidence")
predict(lm.fit,new_values, interval="prediction")
```
...
We can use the `ggfortify` package to produce diagnostic plots. These plots are automatically produced by applying the `autoplot()` function directly to the output from `lm()`.
...
plot2 = ggplot() +
  geom_point(aes(predict(lm.fit), rstudent(lm.fit))) +
